...
//    }
    ...
    public function actionAdd() {
        ...
        $model_ie = new ClientIndividualEntrepreneur();
        $model_mail_adress = new ClientMailingAddress();
        $model_non_resident = new ClientNonResident();
        $model_reason = new ClientReason();
        $model_resident = new ClientResident();
...
//                    $model_clients->load(Yii::$app->request->post()) &&
                    ...
                    && (Yii::$app->session->get('id_type') == 1 ? 
                        $model_resident->load(Yii::$app->request->post()) : 
                            (Yii::$app->session->get('id_type') == 2 ? 
                                ($model_non_resident->load(Yii::$app->request->post()) && $model_country->load(Yii::$app->request->post())): 
                            $model_ie->load(Yii::$app->request->post()))
                       )
                ){
...
//exit();
                    ...
                    $model_clients->type_client = Yii::$app->session->get('id_type');
                ...
                Yii::$app->session->setFlash('success', 'Теперь заполните данные клиента');
                         ...
                         return $this->renderAjax('client/_client', 
                            [
                                ...
                                'model_ie' => $model_ie,
                                'model_mail_adress' => $model_mail_adress, 'model_non_resident' => $model_non_resident,
